...
콘서트 좌석 조회/예약 API 앞단에 **대기열 기반 트래픽 제어**를 붙인 데모 프로젝트입니다.

요청이 평소 처리량 안에 있으면 바로 `consumer-api`로 보내고, 순간 트래픽이 몰리면 `turnstile` 대기열로 보낸 뒤 순서가 된 사용자에게 JWT 입장 토큰을 발급합니다.
